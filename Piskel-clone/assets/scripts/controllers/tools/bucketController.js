export default function bucketController() {
  ...
  function bucketHandler() {
    ...
    canvasWrapper.fillStyle = `rgba${document.getElementById('curr-color').style.backgroundColor.slice(3, -1)}, 255)`;
    if (buttonContainer.getElementsByClassName('active')[0] === buttons[2]) {
      const currCanvas = canvasWrapper.getElementsByClassName('current')[0];
  ...
  return canvasWrapper.addEventListener('mousedown', bucketHandler);
}
/*
